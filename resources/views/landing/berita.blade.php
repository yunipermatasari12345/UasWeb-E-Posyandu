...
<head>
    ...
    <title>Berita Kesehatan - Yuni Permata Sari MI2C</title>
...
<body class="bg-gray-50">
    <header class="bg-white shadow-sm sticky top-0 z-50">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <nav class="hidden md:flex space-x-8">
                ...
                <a href="{{ route('berita') }}" class="text-blue-600 font-bold">Berita Kesehatan</a>
    ...
    <main class="container mx-auto px-4 py-16">
        <h2 class="text-3xl font-bold text-gray-800 mb-6">Berita Kesehatan</h2>
        @if(isset($beritas) && $beritas->count())
            @foreach($beritas as $berita)
                <div class="mb-6 p-4 bg-white rounded shadow">
                    <h3 class="text-xl font-bold mb-1">{{ $berita->judul }}</h3>
                    <div class="text-gray-500 text-sm mb-2">
                        {{ $berita->penulis }} | {{ $berita->created_at->format('d M Y') }}
                    </div>
                    <div class="text-gray-700">{{ Str::limit($berita->isi, 200) }}</div>
                </div>
            @endforeach
            <div class="mt-4">{{ $beritas->links() }}</div>
        @else
            <div class="bg-white rounded-lg shadow p-8 text-gray-600">
                <p>Belum ada berita terbaru. Konten akan segera hadir.</p>
            </div>
        @endif
    </main>
</body>
